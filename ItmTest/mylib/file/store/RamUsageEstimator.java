...
public final class RamUsageEstimator
{
		...
		implements Iterable
	{
		...
		static final boolean $assertionsDisabled = !mylib/file/store/RamUsageEstimator.desiredAssertionStatus();
		...
		public Iterator iterator()
		{
			return new Iterator() {
				...
				{
					this.this$0 = IdentityHashSet.this;
					super();
					pos = -1;
					nextElement = fetchNext();
				}
	...
	public static final class JvmFeature extends Enum
	{
		...
		{
			return (JvmFeature)Enum.valueOf(mylib/file/store/RamUsageEstimator$JvmFeature, name);
		}
				...
				if (!checkInterned || !(ob instanceof String) || ob != ((String)ob).intern())
				{
					...
					} else
					{
						try
						{
							...
							Field afield[] = cachedInfo.referenceFields;
							int j = afield.length;
							for (int k = 0; k < j; k++)
							{
								Field f = afield[k];
	...
	{
		EnumSet unsupported = EnumSet.allOf(mylib/file/store/RamUsageEstimator$JvmFeature);
	...
	{
		return humanReadableUnits(bytes, new DecimalFormat("0.#", new DecimalFormatSymbols(Locale.ENGLISH)));
	}
	...
	/**
	 * @deprecated Method RamUsageEstimator is deprecated
	 */
	...
	/**
	 * @deprecated Method estimateRamUsage is deprecated
	 */
	...
	static 
	{
		...
		supportedFeatures = EnumSet.noneOf(mylib/file/store/RamUsageEstimator$JvmFeature);
		...
		try
		{
			...
			Field dummy1Field = mylib/file/store/RamUsageEstimator$DummyTwoLongObject.getDeclaredField("dummy1");
			int ofs1 = ((Number)objectFieldOffsetM.invoke(theUnsafe, new Object[] {
				dummy1Field
			})).intValue();
			Field dummy2Field = mylib/file/store/RamUsageEstimator$DummyTwoLongObject.getDeclaredField("dummy2");
			int ofs2 = ((Number)objectFieldOffsetM.invoke(theUnsafe, new Object[] {
				dummy2Field
			})).intValue();
			if (Math.abs(ofs2 - ofs1) == 8)
			{
				Field baseField = mylib/file/store/RamUsageEstimator$DummyOneFieldObject.getDeclaredField("base");
