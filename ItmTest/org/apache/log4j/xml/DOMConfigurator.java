...
//			UnrecognizedElementHandler, XMLWatchdog, SAXErrorHandler, Log4jEntityResolver
	...
	implements Configurator
{
	...
	{
		String className;
		className = subst(appenderElement.getAttribute("class"));
	...
	{
		ErrorHandler eh = (ErrorHandler)OptionConverter.instantiateByClassName(subst(element.getAttribute("class")), org.apache.log4j.spi.ErrorHandler.class, null);
		if (eh != null)
		{
			...
			for (int loop = 0; loop < length; loop++)
			{
				...
				if (tagName.equals("logger-ref"))
				{
					...
					Logger logger = catFactory != null ? repository.getLogger(loggerName, catFactory) : repository.getLogger(loggerName);
					eh.setLogger(logger);
					continue;
				}
	...
	protected void parseFilters(Element element, Appender appender)
	{
		...
		Filter filter = (Filter)OptionConverter.instantiateByClassName(clazz, org.apache.log4j.spi.Filter.class, null);
		if (filter != null)
		{
	...
	protected void parseCategory(Element loggerElement)
	{
		...
		if ("".equals(className))
		{
			...
			cat = catFactory != null ? repository.getLogger(catName, catFactory) : repository.getLogger(catName);
		} else
		{
	...
	protected void parseCategoryFactory(Element factoryElement)
	{
		...
		} else
		{
			...
			Object factory = OptionConverter.instantiateByClassName(className, org.apache.log4j.spi.LoggerFactory.class, null);
	...
	{
		String className;
		className = subst(layout_element.getAttribute("class"));
	...
	protected void parse(Element element)
	{
		...
		for (int loop = 0; loop < length; loop++)
		{
			...
			if (tagName.equals("throwableRenderer"))
			{
			...
			if (!tagName.equals("appender") && !tagName.equals("categoryFactory") && !tagName.equals("loggerFactory"))
