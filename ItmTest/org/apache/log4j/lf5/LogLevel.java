	...
	implements Serializable
{
	...
	public static void register(List logLevels)
	{
		if (logLevels != null)
		{
			for (Iterator it = logLevels.iterator(); it.hasNext(); register((LogLevel)it.next()));
		}
	}
	...
	public static void resetLogLevelColorMap()
	{
		...
		for (int i = 0; i < _allDefaultLevels.length; i++)
			_logLevelColorMap.put(_allDefaultLevels[i], java.awt.Color.black);
	...
	static 
	{
		...
		_log4JLevels = (new LogLevel[] {
			FATAL, ERROR, WARN, INFO, DEBUG
		});
		_jdk14Levels = (new LogLevel[] {
			SEVERE, WARNING, INFO, CONFIG, FINE, FINER, FINEST
		});
		_allDefaultLevels = (new LogLevel[] {
			FATAL, ERROR, WARN, INFO, DEBUG, SEVERE, WARNING, CONFIG, FINE, FINER, 
			FINEST
		});
