...
public class ConfigurationManager
{
	...
	protected void processLogTableColumns(Document doc)
	{
		...
		java.util.List selectedColumns = new ArrayList();
		for (int i = 0; i < nodeList.getLength(); i++)
		{
			Node n = nodeList.item(i);
			if (n == null)
	...
	protected void selectAllNodes()
	{
		...
		CategoryNode n = null;
		for (; all.hasMoreElements(); n.setSelected(true))
	...
	private void processConfigurationNode(CategoryNode node, StringBuffer xml)
	{
		...
		CategoryNode n = null;
		for (; all.hasMoreElements(); exportXMLElement(n, model.getTreePathToRoot(n), xml))
			n = (CategoryNode)all.nextElement();
	...
	private void processLogLevels(Map logLevelMenuItems, StringBuffer xml)
	{
		...
		for (Iterator it = logLevelMenuItems.keySet().iterator(); it.hasNext(); exportLogLevelXMLElement(level.getLabel(), item.isSelected(), xml))
		{
			level = (LogLevel)it.next();
			item = (JCheckBoxMenuItem)logLevelMenuItems.get(level);
		}
	...
	private void processLogLevelColors(Map logLevelMenuItems, Map logLevelColors, StringBuffer xml)
	{
		...
		LogLevel level;
		Color color;
		for (Iterator it = logLevelMenuItems.keySet().iterator(); it.hasNext(); exportLogLevelColorXMLElement(level.getLabel(), color, xml))
		{
			level = (LogLevel)it.next();
			color = (Color)logLevelColors.get(level);
		}

		xml.append("\t</loglevelcolors>\r\n");
	}

	private void processLogTableColumns(java.util.List logTableColumnMenuItems, StringBuffer xml)
	{
		xml.append("\t<logtablecolumns>\r\n");
		LogTableColumn column;
		JCheckBoxMenuItem item;
		for (Iterator it = logTableColumnMenuItems.iterator(); it.hasNext(); exportLogTableColumnXMLElement(column.getLabel(), item.isSelected(), xml))
		{
			column = (LogTableColumn)it.next();
			item = _monitor.getTableColumnMenuItem(column);
		}
