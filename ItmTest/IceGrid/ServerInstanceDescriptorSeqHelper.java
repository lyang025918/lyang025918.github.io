...
// Source File Name:   ServerInstanceDescriptorSeqHelper.java
...
public final class ServerInstanceDescriptorSeqHelper
{
	...
	public static void write(BasicStream __os, List __v)
	{
		...
		} else
		{
			...
			ServerInstanceDescriptor __elem;
			for (Iterator i$ = __v.iterator(); i$.hasNext(); __elem.__write(__os))
				__elem = (ServerInstanceDescriptor)i$.next();
	...
	public static List read(BasicStream __is)
	{
		...
		{
			ServerInstanceDescriptor __elem = new ServerInstanceDescriptor();
			__elem.__read(__is);
			__v.add(__elem);
		}
